...
matmodel = '/Users/macx/Desktop/car_final.mat';
xmlfile = '/Users/macx/Desktop/car_final.xml';
...
layer = 1;
%?¡®¡±?¡Ì?¡°?¡Ç?????¡Æ¡Â¡À????¡ã????root filter¡ã?part filter¡ã?deformation filter
for icom = 1:ncom
    fprintf(fid, '\t<Component>\n');
        ...
        %¡®?¡Ì?¡Ç?component???¡§???¡ãpart filter??¡Ç????¡§¡Ü????¡ã¡Ì?¡Ç?part??¡Ü???
        npart = length(model.rules{model.start}(icom).rhs) -1 ;
        fprintf(fid, '\t\t\t<NumPartFilters>%d</NumPartFilters>\n', npart);
        %%???¡ã¡Ì?¡Ç?part????¦Ð?¡Ü???[dx,dy,ds]¡Ò?penalty[dx dy dxx dyy]
        for ipart = 2: npart+1
